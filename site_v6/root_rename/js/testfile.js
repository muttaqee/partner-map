...
function Model() {
  ...
  this.prototype.addTable = function(table) {
...
function loadTables() {
  alert("loadTables"); // FIXME
  // FIXME: Put in build model:
  m = new Model();
  ...
  query(str, function(data) {
    ...
    alert("No. rows: " + rows.length); // FIXME
    for (var i = 0; i < rows.length; i += 1) {
      ...
      alert(id + ", " + name + ", " + label + ", " + type + ", " + is_searchable);
      // alert("No. rows 2: " + rows.length); // FIXME
      var table = new Table(id, name, label, type, is_searchable, rating_table);
      m.addTable(table);
      // alert(JSON.stringify(table));
    }
    alert(JSON.stringify(m.tables)); // FIXME: Remove
  });
