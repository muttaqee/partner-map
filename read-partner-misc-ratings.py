...
"""
Read data for partner_misc_ratings table:
...
x_first = column_index_from_string('CL')
x_last = column_index_from_string('CQ')
y_first = 4
y_last = 106
width = len(columns)
...
def getColNameFromX(x):
    return columns[x - column_index_from_string("CL")]
...
def encodeRow(x, y, rating_value):
    ...
    misc_type = getColNameFromX(x)
    rating = rating_value

    json_row += getNameValuePair("partner_name", "\"" + partner_name + "\"") + ","
    json_row += getNameValuePair("misc_type", "\"" + misc_type + "\"") + ","
