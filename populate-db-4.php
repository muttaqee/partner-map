<?php
  ...
  */
  ...
  // Populate table: consultant_skill_ratings
  function populate_consultant_skill_ratings() {
    global $scripts_path;
    ...
    foreach ($rows as $key => $row) {
      ...
      $sql = "
      SELECT id FROM consultant_skills
      WHERE consultant_skills.name LIKE \"" . $row["area"] . "\"
      LIMIT 1
      ";
  ...
  function populateTables() {
    ini_set('max_execution_time', 300); // Default 30 secs may be too short
    ...
    populate_consultant_skill_ratings();# FIXME: Depends on MAKESHIFT consultants table
