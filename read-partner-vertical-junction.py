...
columns = ["All", "FS", "COM", "HLS", "FED", "RCCM", "SLG", "EN/MFG", "UTL"]
x_first = column_index_from_string('CR')
x_last = column_index_from_string('CZ')
y_first = 4
y_last = 106
width = len(columns)
...
def encodeRow(x, y):
    ...
    vertical = getColNameFromX(x)

    json_row += getNameValuePair("partner_name", "\"" + partner_name + "\"") + ","
    json_row += getNameValuePair("vertical", "\"" + vertical + "\"")
